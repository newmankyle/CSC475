...
public interface VMMPredictor {
  ...
  default double predict(CharSequence symbols, CharSequence context){
      double ans = predict(symbols.charAt(0),context);
      String tempCon = context.toString();
      for(int i = 1; i < symbols.length(); i++){
          tempCon += symbols.charAt(i-1);
          ans *= predict(symbols.charAt(i),tempCon);
      }
      return ans;
  }
